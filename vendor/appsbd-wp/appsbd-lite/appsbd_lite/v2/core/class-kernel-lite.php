...
if ( ! class_exists( __NAMESPACE__ . '\Kernel_Lite' ) ) {
	...
	abstract class Kernel_Lite {
		...
		final public function __construct( $plugin_file ) {
			...
			spl_autoload_register( array( $this, '_vendor_method' ) );
		...
		public function wp_admin_check_default_css_script( $src ) {
			...
			if ( empty( $src ) || 1 == $src || preg_match( '/\/uilib|\/css\/all-css.css|apbd\-|\/wp-admin\/|\/wp-includes\/|\/plugins\/woocommerce\/assets\/|\/plugins\/elementor\/assets\/css\/admin/', $src ) ) {
				return true;
			}
		...
		/**
		 * The  vendor method is generated by appsbd
		 *
		 * @param mixed $class_name_space Its class_name_space param.
		 */
		public function _vendor_method( $class_name_space ) {
			$path  = realpath( __DIR__ . '/../../../' ) . DIRECTORY_SEPARATOR;
			$path2 = realpath( plugin_dir_path( $this->plugin_file ) );
			if ( strpos( $path, $path2 ) !== false ) {
				$this->_load_class( $path, $class_name_space );
			}
		}
		...
		/**
		 * The  load class is generated by appsbd
		 *
		 * @param mixed $path Its the path.
		 * @param mixed $class_name_space Its class_name_space param.
		 */
		protected function _load_class( $path, $class_name_space ) {
		...
		public function lite_admin_script() {
			...
			foreach ( $wp_scripts->queue as $script ) {
				if ( ! in_array( $script, self::$appsbd_global_js ) ) {
					if ( $this->is_global_asset( $script, $wp_scripts->registered[ $script ]->src ) ) {
						continue;
					}
		...
		public function lite_admin_style() {
			...
			foreach ( $wp_styles->queue as $style ) {
				if ( ! in_array( $style, self::$appsbd_global_css ) ) {
					if ( $this->is_global_asset( $style, $wp_styles->registered[ $style ]->src ) ) {
						continue;
					}
